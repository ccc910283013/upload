...
import com.ewell.upload.quartz.util.FybUploadJob;
...
public class JobRunner implements ApplicationRunner {
    ...
    //@Value("${job.Context}")
    //private String jobContext;//获取job清单
    @Resource
    ...
    public void run(ApplicationArguments args) throws Exception {
        ...
        JobDetail jobDetail = JobBuilder.newJob(FybUploadJob.class).
                withIdentity(JOB_NAME + "upload").build();
        Trigger cronTrigger = TriggerBuilder.newTrigger()
                .withIdentity(JOB_NAME + "upload")
                .startNow()
                .withSchedule(CronScheduleBuilder.cronSchedule(environment.getProperty("cronExpression")))
                .build();
...
//        if (jobEnable){
//            List<String> taskList = Arrays.asList(jobContext.split(","));//获取job结果集
